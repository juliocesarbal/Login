...
import {
  faEnvelope, faUser, faEye, faEyeSlash, faAddressCard
} from "@fortawesome/free-regular-svg-icons";
import {
  faLock, faHouse, faPhone, faPerson, faShop
...
const ModalCreateUser = ({ onClose, onUserCreated }) => {
  ...
  const validations = (name, value) => {
    ...
    } else if (name === "password" && (value.length < 6 || !/[A-Z]/.test(value))) {
  ...
  return (
    <div className="modal-register-backdrop">
        <form className="modal-register-content" onSubmit={handleSubmit}>
          ...
          {[
            { icon: faAddressCard, name: "ci", placeholder: "Cédula de Identidad" },
            { icon: faUser, name: "name", placeholder: "Nombre de usuario" },
            { icon: faPhone, name: "telefono", placeholder: "Teléfono" },
            { icon: faEnvelope, name: "email", placeholder: "Correo electrónico", type: "email" },
          ...
          <div className="modal-Inputclass">
            ...
            <select name="id_sucursal" value={objData.id_sucursal} onChange={handleChange}>
          ...
          <div className="button-group-createUser">
            <button type="submit" disabled={isDisabled} id="buttonLogIn">Crear</button>
